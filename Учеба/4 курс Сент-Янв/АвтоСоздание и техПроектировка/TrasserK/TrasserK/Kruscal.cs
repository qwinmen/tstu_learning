...
using System.Linq;
using System.Text;
//http://msugvnua000.web710.discountasp.net/Posts/Details/3751
namespace TrasserK
{
    class Kruskal
    {
            ...
            for (i = 1; i <= k; i++)
            {
                for (i = 1; i < k; i++)
                    if (this.Find(_edges[i].U) != this.Find(_edges[i].V))
                    {
                        ...
                        this.Join(Find(_edges[i].U), Find(_edges[i].V));//this.Join(_edges[t].U, _edges[t].V);
                        t++;
                    }
            }
        }
        ...
        {
            Console.WriteLine("Минимальная стоимость при соединении вершин:");
            for (int i = 1; i < _verticlesCount; i++)
                Console.WriteLine(tree[i, 1] + " с " + tree[i, 2]);
        }
