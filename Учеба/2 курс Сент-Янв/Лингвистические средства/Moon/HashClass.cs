...
namespace Moon
{
    class HashClass
    {
        ...
        void TxtFromTextBoxIn()
        {
            ...
            {
                Console.WriteLine("Повторение по ID");//MessageBox.Show("Повторение по ID");
                _Id[obshIndex] = "*";
                _Type[obshIndex] = words[0];
                _Hash[obshIndex] = -42;
                obshIndex++;
                Program.flag=false;//FormMain.flag = false;
            }
        }
        private void Param(int keyHash, int index, bool flag, int ent)
        {
            Program.key = keyHash;//formMain.key = keyHash;
            Program.index = index;//formMain.index = index;
            Program.flag = flag;
            Program.ent = ent;
        }
