...
namespace Ganza
{
    public partial class FormGanza : Form
    {
        public FormGanza()
        {
            ...
            _pen = new Pen(_colorColorSelector);
        ...
        private void pictureBox1_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                {//Триугольник
                    ...
                    Vertexes[2] = GetThirdVertex(Vertexes[0], Vertexes[1]);//Считаем третью вершину
            ...
            {//Эллипсис
                ...
                x4 = x1;//начало
                y4 = y3;
                gr.DrawEllipse(new Pen(Color.Black),x1,y1,e.X-x1,e.Y-y1 );
            } 
             * 
             * Graphics graf = pictureBoxHolst.CreateGraphics();
                  if (i & !key)
                  {//карандаш
                      x2 = e.X;//конец=конец
                      y2 = e.Y;//конец=конец
                      graf.DrawLine(new Pen(colorColorSelector, penSize), _downPoint.X, _downPoint.Y, x2, y2);
                      x1 = x2;//начало=конец
                      y1 = y2;//начало=конец
                  }else if (i & key)
              {//прямоугольник
                  graf.DrawRectangle(new Pen(colorColorSelector, penSize), x1, y1, ox - x1, oy - y1);
                  pictureBoxHolst.Refresh();
                  graf.DrawRectangle(new Pen(colorColorSelector, penSize), x1, y1, e.X - x1, e.Y - y1);
                  ox = e.X;
                  oy = e.Y;
              }
            */
        ...
        //List<GraphicsPath> triangles = new List<GraphicsPath>();//Области пересечения треугольников
