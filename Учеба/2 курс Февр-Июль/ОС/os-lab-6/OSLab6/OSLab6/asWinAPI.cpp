...
VOID SetGlobalError(LPCSTR sFuncFailed)
{
...
HANDLE WINAPI SCreateMutex(LPCSTR name, BOOL bInitialOwner)
{
	HANDLE hMutex = CreateMutexA(NULL, bInitialOwner, name);
	if (hMutex == NULL)
		SetGlobalError("global:CreateMutex");
	return hMutex;
}

HANDLE WINAPI SOpenMutex(LPCSTR name)
{
	HANDLE hMutex = OpenMutexA(MUTEX_ALL_ACCESS, FALSE, name);
	if (hMutex == NULL)
		SetGlobalError("global:OpenMutex");
	return hMutex;
}
...
VOID WINAPI SCloseHandle(HANDLE handle)
{
	if (CloseHandle(handle) == FALSE)
		SetGlobalError("global:CloseHandle");
}
...
HANDLE WINAPI SCreateEvent(LPCSTR lpEventName)
{
	HANDLE hEvent = CreateEventA(NULL, TRUE, FALSE, lpEventName);
	if (hEvent == NULL)
		SetGlobalError("global:CreateEvent");
	return hEvent;
}

HANDLE WINAPI SOpenEvent(LPCSTR lpEventName)
{
	HANDLE hEvent = OpenEventA(EVENT_ALL_ACCESS, FALSE, lpEventName);
	if (hEvent == NULL)
		SetGlobalError("global:OpenEvent");
	return hEvent;
}
